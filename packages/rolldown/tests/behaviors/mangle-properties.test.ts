import type { ManglePropertiesOptions, OutputChunk, Plugin } from '@rollipop/rolldown';
...
test.skip('rejects property mangling for multiple output chunks', async () => {
...
test.skip('rejects property mangling with an emitted prebuilt chunk', async () => {
...
test.skip('mangles selected properties and honors reserved and cached names', async () => {
  const modules = {
    'entry.js':
...
test.skip('returns a complete reusable property mangle cache', async () => {
...
test.skip('distinguishes disabled property mangling from an empty cache', async () => {
  const bundle = await rolldown({
    input: 'entry',
