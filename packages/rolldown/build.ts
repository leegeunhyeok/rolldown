...
function generateRuntimeEntry() {
  const runtimeTypesInputFile = nodePath.resolve(
    __dirname,
  ...
  console.log(styleText('green', '[build:done]'), 'Generating dts from', runtimeTypesInputFile);

  const commonRuntimeSource = fs.readFileSync(runtimeTypesInputFile, 'utf-8');
  ...
  const result = ts.transpileDeclaration(commonRuntimeSource, {
    compilerOptions: {
      ...getTsconfigCompilerOptionsForFile(runtimeTypesInputFile),
      noEmit: false,
      emitDeclarationOnly: true,
    },
    fileName: runtimeTypesInputFile,
  });
